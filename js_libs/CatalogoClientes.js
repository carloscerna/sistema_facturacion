// id de user global
var id_clientes = 0;
var accion_cliente = 'noAccion';
var fecha_creacion_cliente = "";
var codigo = "";
var ann = ""; // valor de anno actual para generar nuevamente los c{odigos del los cliente/empresa.
...
///////////////////////////////////////////////////////////////////////////////	  
$('#listadoClientes').on( 'click', 'a.remove', function () {
			id_clientes =$(this).parents('tr').find('td').eq(0).html();
			codigo =$(this).parents('tr').find('td').eq(1).html();
			cliente_empresa =$(this).parents('tr').find('td').eq(2).html();
			accion_cliente = "EliminarRegistro";
			// Abrimos el Formulario Modal y Rellenar For.
				$('#VentanaEliminar').modal("show");
				$("span[for='Id']").text(id_clientes);
...
$("#BotonEliminarRegistro").on('click', function(){
	// proceso para eliminar el registro cargar variables públicas.
				$.ajax({
		            ...
		            dataType: "json",
                        url:"php_libs/soporte/CatalogoClientes.php",
		            data:"accion_cliente_buscar="+ accion_cliente + "&id_=" + id_clientes + "&codigo_cliente=" + codigo + "&id=" + Math.random(),
		            success: function(response){
                        ...
                        // Si el valor si existe compararlo con mensaje error.
                           ...
                           if (response.mensaje == "No Eliminar") {
                                alertify.error("No se puede Eliminar El Cliente. Tiene Facturas en el Sistema.");
                           }
                           if (response.mensaje == "No Registro") {
                                alertify.error("Registro(s) No Eliminado(s).");
                           }
		            },
		            error:function(){
	...
	accion_cliente = ""; id_clientes = 0; codigo = ""; cliente_empresa = "";
...
///////////////////////////////////////////////////////////////////////////////	  
$('#listadoClientes').on( 'click', 'a.editar', function () {
				id_clientes =$(this).parents('tr').find('td').eq(0).html();
				accion_cliente = "EditarRegistro";	// variable global
				$.post("php_libs/soporte/CatalogoClientes.php", {accion_cliente_buscar: accion_cliente, id_: id_clientes},
				       function(data){
...
///////////////////////////////////////////////////////////////////////////////
// BOTÓN GUARDAR. QUE ESTA DENTRO DEL FORM AGREGARUSER O ACTUALIZAR
///////////////////////////////////////////////////////////////////////////////	
		$('#formClienteEmpresa').validate({
			rules:{
			        txtClienteEmpresa: {
						required: true,
						maxlength: 120
		    ...
		    submitHandler: function(){
             // Serializar los datos, toma todos los Id del formulario con su respectivo valor.
		        var str = $('#formClienteEmpresa').serialize();
		        ...
		        $.ajax({
		            ...
		            dataType: "json",
		            url:"php_libs/soporte/CatalogoClientes.php",
		            data:str + "&accion_cliente_buscar=" + accion_cliente + "&id=" + Math.random() + "&id_=" + id_clientes + "&ann="+ann,
		            success: function(response){
                               // Si el valor si existe compararlo con mensaje error.
								...
								// Limpiar variables Text, y textarea
									$("#formClienteEmpresa")[0].reset();
								// Abrimos el Formulario Modal y Rellenar For.
									$('#VentanaClienteEmpresa').modal("hide");
									listar();
                                }
...
///////////////////////////////////////////////////
		$('#NuevoCliente').on('click',function(){
			// Acción nuevo registro y variables default.
				accion_cliente = "GenerarCodigoNuevo";
				$('#txtFechaCreacion').val(fecha_creacion_cliente);
			// Generar el Código Nuevo.
			$.post("php_libs/soporte/CatalogoClientes.php", {accion_cliente_buscar: accion_cliente, ann: ann},
				function(data){
			...
			// cambiar aacion_cliente para poder guardar el registro.
				accion_cliente = "GuardarRegistro";
		});
		///////////////////////////////////////////////////
		// funcionalidad del botón que abre el formulario
		///////////////////////////////////////////////////
	    $("#VentanaClienteEmpresa").on('hidden.bs.modal', function () {
            // Limpiar variables Text, y textarea
				$("#formClienteEmpresa")[0].reset();
				$("label.error").remove();
				// Desabilitar Fecha Creación Proveedor.
		...
		///////////////////////////////////////////////////
			$('#VentanaClienteEmpresa').on('shown.bs.modal', function() {
