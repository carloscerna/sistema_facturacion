...
if($errorDbConexion == false){
	...
	if(isset($_POST) && !empty($_POST)){
		...
		case 'BuscarTodos':
				// Armamos el query.
				$query = "SELECT fac_t.id_factura_tiraje,  fac_t.tiraje, fac_t.numero_inicio, fac_t.numero_fin, fac_t.codigo_tipo_factura, cat_fac.descripcion as nombre_tipo_factura,
                            to_char(fac_t.fecha_autorizacion,'DD/MM/YYYY') as fecha_autorizacion, to_char(fac_t.fecha_impresion,'DD/MM/YYYY') as fecha_impresion,
                            cat_est.descripcion as nombre_estatus
                            FROM facturas_tiraje fac_t
                            INNER JOIN catalogo_tipo_factura cat_fac ON cat_fac.codigo = fac_t.codigo_tipo_factura
                            INNER JOIN catalogo_estatus cat_est ON cat_est.codigo = fac_t.codigo_estatus
                            ORDER BY fac_t.id_factura_tiraje ASC";
				// Ejecutamos el Query.
			...
			case 'GuardarRegistro':
                ...
                if($consulta_estatus -> rowCount() != 0){
                    ...
                    ///
                    $query = "INSERT INTO facturas_tiraje (codigo_tipo_factura, codigo_estatus, tiraje, numero_inicio, numero_fin, fecha_autorizacion, fecha_impresion)
